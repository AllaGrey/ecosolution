...
export const CasesSlider = () => {
  ...
  const [currentSlideIndices, setCurrentSlideIndices] = useState(() =>
    generateInitialIndices(itemsPerPage)
  );
  ...
  const lastSlideId =
    SLIDER_DATA[currentSlideIndices[currentSlideIndices.length - 1]].id;
    ...
    setCurrentSlideIndices(prevIndices => {
      return prevIndices.map(index =>
        index - itemsPerPage >= 0
          ? index - itemsPerPage
          : SLIDER_DATA.length - itemsPerPage + index
      );
    });
  };
  ...
  const goToNextSlide = () => {
    setCurrentSlideIndices(prevIndices => {
      return prevIndices.map(index =>
        index + 1 < SLIDER_DATA.length ? index + 1 : 0
      );
    });
  };
